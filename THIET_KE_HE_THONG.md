...
| --- | --- | --- | --- | --- |
| GAP-01 | `stock_adjustments` | Phiếu tạo tay (`POST /stock-adjustments`) sinh ra ở `DRAFT`, nhưng `approve` và `reject` chỉ chấp nhận `PENDING`, và **không có API nào chuyển `DRAFT` sang `PENDING`** | Phiếu điều chỉnh thủ công tạo xong chỉ có thể hủy, không thể duyệt. Chỉ phiếu sinh từ kiểm kê (loại `COUNT`) mới vào được `PENDING` | Bổ sung `POST /stock-adjustments/:id/submit`, hoặc cho `approve` nhận cả `DRAFT` |
