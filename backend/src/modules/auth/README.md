...
- Toàn bộ schema nằm trong một file duy nhất `backend/warehouse_management_mysql.sql`, không có thư mục migration riêng. Nếu database đang chạy thiếu bảng `password_reset_requests` hoặc quyền `users:reset_password` (triệu chứng: `GET /auth/password-reset/requests` trả 500 kèm log `Table ... doesn't exist`), chép ba khối tương ứng trong file đó sang chạy tay: `CREATE TABLE password_reset_requests`, dòng seed `('users:reset_password', ...)` trong `INSERT INTO permissions`, và `'users:reset_password'` trong `INSERT INTO role_permissions`.
